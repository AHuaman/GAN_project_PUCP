...
class MidiNet(object): ##Model 1, no 1D conditioner (chords), includes 2D conditioner (previous melody)
    def __init__(self, sess, is_crop=False,
                 ...
                 checkpoint_dir=None, sample_dir=None, gen_dir= None): #These should indicate the folders for export and saving
        self.sess = sess
    ...
    def build_model(self):
        ...
        self.D, self.D_logits, self.fm = self.discriminator(self.images, reuse=False)

        self.sampler = self.sampler(self.z,self.prev_bar)
        self.D_, self.D_logits_ , self.fm_ = self.discriminator(self.G, reuse=True)
    ...
    def train(self, config):
        ...
        if config.dataset == 'MidiNet_v1':
            ...
            data_X, prev_X, data_y = shuffle(data_X,prev_X, random_state=0)
        ...
        for epoch in range(config.epoch):
            ...
            for idx in range(0, batch_idxs):
                ...
                _, summary_str = self.sess.run([d_optim, self.d_sum],
                    feed_dict={ self.images: batch_images, self.z: batch_z ,self.prev_bar:prev_batch_images })
                ...
                errD_fake = self.d_loss_fake.eval({self.z: batch_z, self.prev_bar:prev_batch_images })
                errD_real = self.d_loss_real.eval({self.images: batch_images, })
                errG = self.g_loss.eval({self.images: batch_images, self.z: batch_z, self.prev_bar:prev_batch_images })
                ...
                if np.mod(counter, 100) == 1:
                    ...
                    samples, d_loss, g_loss = self.sess.run(
                        ...
                        feed_dict={self.z: sample_z, self.images: sample_images, self.prev_bar:prev_batch_images }
                    )
                    #samples = (samples+1.)/2.
    ...
    def discriminator(self, x, reuse=False):
        ...
        h0 = lrelu(self.d_bn0(conv2d(x, df_dim, k_h=4, k_w=89, name='d_h0_conv')))
        fm = h0 #New line added, simple feature matching vector
        h1 = lrelu(self.d_bn1(conv2d(h0, df_dim, k_h=4, k_w=1, name='d_h1_conv')))
        h2 = lrelu(self.d_bn2(conv2d(h1, df_dim, k_h=4, k_w=1, name='d_h2_conv')))
    ...
    def sampler(self, z, y=None, prev_x=None): #This is the same as Generator, not trained and reusing variables
        tf.get_variable_scope().reuse_variables()
        h0_prev = lrelu(self.g_prev_bn0(conv2d(prev_x, 16, k_h=1, k_w=128,d_h=1, d_w=2, name='g_h0_prev_conv')))
        h1_prev = lrelu(self.g_prev_bn1(conv2d(h0_prev, 16, k_h=2, k_w=1, name='g_h1_prev_conv')))
        h2_prev = lrelu(self.g_prev_bn2(conv2d(h1_prev, 16, k_h=2, k_w=1, name='g_h2_prev_conv')))
